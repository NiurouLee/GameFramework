...
---@class PetObtainAnim1Star:PetObtainAnimBase
_class("PetObtainAnim1Star", PetObtainAnimBase)
PetObtainAnim1Star = PetObtainAnim1Star

function PetObtainAnim1Star:Constructor(pet, anim, atlas, uiStars, uiDepth, matAnim)
    ---@type ObtainPet
    self._pet = pet
...
function PetObtainAnim1Star:_GetStaticBody(obtainPet)
    if obtainPet then
        if obtainPet:SkinID() > 0 then
            ...
            if cg then
                ...
                Log.fatal("### [error] pet obtain anim 1 star get cg fail. id = [", obtainPet:PetID(), "]")
            end
        end
    end
    return ""
end
function PetObtainAnim1Star:_GetSpine(obtainPet)
    if obtainPet then
        if obtainPet:SkinID() > 0 then
            local cfgv = Cfg.cfg_pet_skin[obtainPet:SkinID()]
            if cfgv then
...
function PetObtainAnim1Star:Prepare()
...
function PetObtainAnim1Star:_load(TT)
    self._state = ObtainAnimState.Prepare
...
function PetObtainAnim1Star:Start()
    self._anim:Play("uieff_uipetobtain_hide")
    self:checkReady()
end

function PetObtainAnim1Star:checkReady()
...
function PetObtainAnim1Star:Update(dtMS)
...
function PetObtainAnim1Star:IsOver()
    return self._state == ObtainAnimState.Finished
end

function PetObtainAnim1Star:Dispose()
    if self._state == ObtainAnimState.Wait then
...
function PetObtainAnim1Star:InitData()
...
function PetObtainAnim1Star:OnUIDepthChanged(depth)
    depth = depth * 10
    ---@type UIView
    local uiView = self._eff:GetComponent(typeof(UIView))
    --设置相机深度
