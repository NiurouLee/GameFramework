---@class UISetAutoFightCountAndTicketNew:UIController
_class("UISetAutoFightCountAndTicketNew", UIController)
UISetAutoFightCountAndTicketNew = UISetAutoFightCountAndTicketNew

function UISetAutoFightCountAndTicketNew:OnShow(uiParams)
...
function UISetAutoFightCountAndTicketNew:ChangePhysicalPowerNumber()
    self:RefreshPowerCount()
    self:RefreshUI()
end

function UISetAutoFightCountAndTicketNew:Refresh()
    self:RefreshPowerCount()
...
function UISetAutoFightCountAndTicketNew:RefreshTicketCount()
    ---@type ResDungeonModule
    local resModule = self:GetModule(ResDungeonModule)
    self._totalTicket = resModule:GetDoubleResNum()
end

function UISetAutoFightCountAndTicketNew:RefreshPowerCount()
    ---@type RoleModule
    local roleModule = GameGlobal.GetModule(RoleModule)
    self._totalPower = roleModule:GetAssetCount(self._powerID)
end

function UISetAutoFightCountAndTicketNew:CalcTicketAndPower()
...
function UISetAutoFightCountAndTicketNew:CalcAutoFightData()
    local autoDatas = {}
...
function UISetAutoFightCountAndTicketNew:RefreshUI()
    ...
    self:RefreshPowerInfo(false)
    self._ticketCountLabel:SetText("<color=#db9d59>".. self._needTicket .. "</color>/" .. self._totalTicket)
    self._rewardMultipleLabel:SetText(StringTable.Get("str_battle_auto_battle_reward_multiple", self._rewardMultiple))
end

function UISetAutoFightCountAndTicketNew:RefreshPowerInfo(isPress)
    local str = ""
    if isPress then
...
function UISetAutoFightCountAndTicketNew:MaskOnClick()
    self:CloseDialog()
end

function UISetAutoFightCountAndTicketNew:BtnStartOnClick()
    --体力不够打开恢复棱镜界面
    if self._needPower > self._totalPower then
...
function UISetAutoFightCountAndTicketNew:BtnTipsOnClick()
    self:ShowDialog("UISetAutoFightIntroduce")
end

function UISetAutoFightCountAndTicketNew:BtnNoneTicketOnClick()
    self._ticketCount = 0
    self:Refresh()
end

function UISetAutoFightCountAndTicketNew:BtnOneTicketOnClick()
    self._ticketCount = 1
    self:Refresh()
end

function UISetAutoFightCountAndTicketNew:BtnTwoTicketOnClick()
    self._ticketCount = 2
    self:Refresh()
end

function UISetAutoFightCountAndTicketNew:MinBtnOnClick()
    self._fightCount = 1
    self:Refresh()
end

function UISetAutoFightCountAndTicketNew:SubBtnOnClick()
...
function UISetAutoFightCountAndTicketNew:AddBtnOnClick()
...
function UISetAutoFightCountAndTicketNew:MaxBtnOnClick()
    self._fightCount = 0
...
function UISetAutoFightCountAndTicketNew:GetMatchType()
    return self._matchType
end

function UISetAutoFightCountAndTicketNew:GetNeedPower()
    return self._oneBattleNeedPower
end

function UISetAutoFightCountAndTicketNew:GetDoubleTicket()
    return self._doubleTicket
end

function UISetAutoFightCountAndTicketNew:GetUuid()
    return self._uuid
end

function UISetAutoFightCountAndTicketNew:ChkAutoPick()
