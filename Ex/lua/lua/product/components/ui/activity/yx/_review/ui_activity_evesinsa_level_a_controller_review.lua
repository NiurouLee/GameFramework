---@class UIActivityEveSinsaLevelAController_Review:UIController
    ...
    if res and res:GetSucc() then
        ...
        if
            ...
            not self._campaign:CheckComponentOpen(
                ECampaignReviewEvaRescuePlanComponentID.ECAMPAIGN_REVIEW_EVARESCUEPLAN_LINE_MISSION -- 只检查线性关卡是否开启
            )
        then
...
function UIActivityEveSinsaLevelAController_Review:_OpenUIRecord()
    ...
    if data:P1SStageUnLock() then
        if LocalDB.GetInt("ACTIVITY_EVE_SINA_P1S_NEWFLAG_REVIEW" .. loginModule:GetRoleShowID(), 0) <= 0 then
