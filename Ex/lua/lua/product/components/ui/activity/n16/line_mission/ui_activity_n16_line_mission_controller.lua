---@class UIActivityN16LineMissionController:UIController
_class("UIActivityN16LineMissionController", UIController)
UIActivityN16LineMissionController = UIActivityN16LineMissionController

--region Helper
function UIActivityN16LineMissionController:_SpawnObject(widgetName, className)
...
function UIActivityN16LineMissionController:_SetRemainingTime(widgetName, descId, endTime, tickCallback, stopCallback)
...
function UIActivityN16LineMissionController:_SetRemainingTime_Action(
    widgetName,
    descId,
    endTime,
    tickCallback,
...
function UIActivityN16LineMissionController:_SetIcon(widgetName, icon)
...
function UIActivityN16LineMissionController:_SetText(widgetName, str)
...
function UIActivityN16LineMissionController:Constructor()
    ---@type MissionModule
...
function UIActivityN16LineMissionController:InitWidget()
...
function UIActivityN16LineMissionController:LoadDataOnEnter(TT, res, uiParams)
    self._campaignType = ECampaignType.CAMPAIGN_TYPE_N16
    self._componentId_LineMission = ECampaignN16ComponentID.ECAMPAIGN_N16_LEVEL_COMMON
    self._componentId_LineMissionFixteam = ECampaignN16ComponentID.ECAMPAIGN_N16_LEVEL_FIXTEAM
...
function UIActivityN16LineMissionController:OnShow(uiParams)
    self._isOpen = true
    self._timerHolder = UITimerHolder:New()

    UIActivityN16LineMissionController.SLeval = 999 --s关枚举id
    UIActivityN16LineMissionController.Passed = 888 --通关后文本和阴影颜色
    UIActivityN16LineMissionController.NodeCfg = {
        ...
        [UIActivityN16LineMissionController.SLeval] = {
            [1] = {
                normal = "n16_xxg_btn04",
                press = "",
                lock = "",
...
function UIActivityN16LineMissionController:OnHide()
    UIActivityN16LineMissionController.SLeval = nil
    UIActivityN16LineMissionController.NodeCfg = nil
    self._isOpen = false
    ...
    UIActivityN16LineMissionController.super:Dispose()
    self._scroller:Dispose()
end

function UIActivityN16LineMissionController:_Refresh()
    self:FlushNodes()

    self:_SetTimeInfo()
    self._endTime_ActionPoint = nil -- 通过 _Refresh() 方式将强制设置行动点的倒计时
   -- self:_SetActionPoint()
    self:_SetTryoutBtn()
    self:_SetExchangeBtn()
end

function UIActivityN16LineMissionController:_SetTimeInfo()
    local endTime = self._line_component:GetComponentInfo().m_close_time
    self:_SetRemainingTime("_remainingTimePool", nil, endTime)
end

function UIActivityN16LineMissionController:_SetActionPoint()
    ...
    if self._endTime_ActionPoint == endTime then
        Log.info("UIActivityN16LineMissionController:_SetActionPoint() Same endTime")
    elseif n1 >= n2 then
        Log.info("UIActivityN16LineMissionController:_SetActionPoint() n1 >= n2")
    else
        self._endTime_ActionPoint = endTime -- 通过记录结束时间规避循环调用
        self:_SetRemainingTime_Action(
            "_actionPointPool",
            "str_activity_common_next_action_point",
            endTime,
            nil,
            function()
...
function UIActivityN16LineMissionController:_ReloadAndRefreshActionPoint()
    Log.info("UIActivityN16LineMissionController:_ReloadAndRefreshActionPoint()")
    self:StartTask(
        function(TT)
            YIELD(1)
            local res = AsyncRequestRes:New()
            self._campaign:ReLoadCampaignInfo_Force(TT, res)
            -- self._endTime_ActionPoint
...
function UIActivityN16LineMissionController:_SetTryoutBtn()
...
function UIActivityN16LineMissionController:_SetExchangeBtn()
    ...
    local fmtStr = string.format("<color=#4c4c4c>%s</color><color=#fcfbfb>%s</color>", preZero, tostring(count))
...
function UIActivityN16LineMissionController:FlushNodes()
    ...
    self._nodesPool:SpawnObjects("UIActivityN16LineMissionMapNode", levelCount)
    ...
    for missionID, cfg in pairs(showMission) do
        ---@type UIActivityN16LineMissionMapNode
...
function UIActivityN16LineMissionController:_onNodeClick(stageId, isStory, worldPos)
    if isStory then
        --剧情关
...
function UIActivityN16LineMissionController:_EnterStage(stageId, worldPos)
...
function UIActivityN16LineMissionController:PlotEndCallback(stageId)
...
function UIActivityN16LineMissionController:_CheckSerialAutoFightShow(stageType, stageId)
...
function UIActivityN16LineMissionController:ShowSerialRewards()
    self:ShowDialog("UISerialAutoFightInfo", OpenUISerialFightInfoState.Finished)
end

--region Event Callback
--活动说明
function UIActivityN16LineMissionController:ActionPointBtnOnClick(go)
    local componentId = ECampaignN16ComponentID.ECAMPAIGN_N16_ACTION_POINT

    ---@type ActionPointComponent
    local component = self._campaign:GetComponent(componentId)

    self:ShowDialog("UIActivityN16ActionPointDetail", component:GetItemIcon())
end
--endregion

--region AttachEvent
function UIActivityN16LineMissionController:AttachEvents()
    self:AttachEvent(GameEventType.ActivityCloseEvent, self._CheckActivityClose)
    self:AttachEvent(GameEventType.OnUIGetItemCloseInQuest, self.OnUIGetItemCloseInQuest)
end

function UIActivityN16LineMissionController:_CheckActivityClose(id)
    if self._campaign and self._campaign._id == id then
        self:SwitchState(UIStateType.UIMain)
    end
end

function UIActivityN16LineMissionController:OnUIGetItemCloseInQuest(type)
