---@class UIN10TotalLoginAwardController : UIController
_class("UIN10TotalLoginAwardController", UIController)
UIN10TotalLoginAwardController = UIN10TotalLoginAwardController

function UIN10TotalLoginAwardController:Constructor()
    -- self._animInfo = {
...
function UIN10TotalLoginAwardController:_InitCmpt(TT, res, forceSend)
...
function UIN10TotalLoginAwardController:OnAutoPopOpenFail()
    if self.isAutoPopShow then
        GameGlobal.EventDispatcher():Dispatch(GameEventType.MainLobbyAutoOpenTryFail)
    end
end
function UIN10TotalLoginAwardController:CloseDialogWithAnim()
    --if self._rootAnim then
    --self:Lock("UIN10TotalLoginAwardController:CloseDialogWithAnim")
    -- if self._rootAnim then
    ...
    --         self:UnLock("UIN10TotalLoginAwardController:CloseDialogWithAnim")
    --         self:CloseDialog()
    --     end,
    --     self
    -- )
...
function UIN10TotalLoginAwardController:OnShow(uiParams)
    local clientCfg = Cfg.cfg_activity_total_login_client[self._componentFullId]
    if clientCfg then
        --是否是最后一天固定显示
        self._showLast = clientCfg.ShowLastAward or false
    else
        Log.error("###[UIN10TotalLoginAwardController] clientCfg is nil ! id --> ", self._componentFullId)
    end
    self:InitWidget()
    self:_InitScrollPos()
    self:AddListener()
end
function UIN10TotalLoginAwardController:OnHide()
...
function UIN10TotalLoginAwardController:AddListener()
...
function UIN10TotalLoginAwardController:DetachListener()
...
function UIN10TotalLoginAwardController:OnActivityCloseEvent(id)
    if self._campaign and self._campaign._id == id then
        self:SwitchState(UIStateType.UIMain)
    end
end
function UIN10TotalLoginAwardController:OnActivityTotalAwardCellSelect(days)
    self._curSelectedDayNum = days
end
function UIN10TotalLoginAwardController:LoadDataOnEnter(TT, res, uiParams)
    if uiParams[1] then
...
function UIN10TotalLoginAwardController:InitWidget()
    --generated--
    self._refreshTaskID = nil
...
function UIN10TotalLoginAwardController:_FillCfgUiRes()
...
function UIN10TotalLoginAwardController:CloseBtnOnClick(go)
    self:CloseDialogWithAnim()
end
function UIN10TotalLoginAwardController:_InitData()
    self._data = {}
    self._data.cells = {}
...
function UIN10TotalLoginAwardController:_InitDayRewardList()
...
function UIN10TotalLoginAwardController:_OnGetRewardCell(scrollview, index)
    ...
    if item.IsInitHandlerCalled == false then
        ...
        cellPool:SpawnObjects("UIN10TotalLoginAwardCell", 1)
    end
...
function UIN10TotalLoginAwardController:ShowItemInfo(matid, pos)
    self._tips:SetData(matid, pos)
end
function UIN10TotalLoginAwardController:_InitScrollPos()
...
function UIN10TotalLoginAwardController:_MoveScrollToItemIndex(itemIndex)
    self._dayRewardList:MovePanelToItemIndex(itemIndex, 0)
    self._dayRewardList:FinishSnapImmediately()
end
function UIN10TotalLoginAwardController:_GetFirstShowItemIndex()
    local cellIndex = 1
...
function UIN10TotalLoginAwardController:GetTotalAward(days)
    self:Lock("UIN10TotalLoginAwardController:GetTotalAward(id)")
    self:StartTask(self.OnGetTotalAward, self, days)
end
function UIN10TotalLoginAwardController:OnGetTotalAward(TT, days)
    if self._cumulativeLoginCmpt then
        local res = AsyncRequestRes:New()
        local awards = self._cumulativeLoginCmpt:HandleReceiveCumulativeLoginReward(TT, res, days)
        self:UnLock("UIN10TotalLoginAwardController:GetTotalAward(id)")
        if res == nil then
            return
        end
        if res:GetSucc() then
...
function UIN10TotalLoginAwardController:_ShowAwards(awards, days)
    local tempPets = {}
    if #awards > 0 then
        for i = 1, #awards do
...
function UIN10TotalLoginAwardController:_SetListCellCount()
    self._dayRewardList:SetListItemCount(#self._data.cells, false)
end
function UIN10TotalLoginAwardController:_ForceRefresh()
    self._refreshTaskID =
        self:StartTask(
        function(TT)
            local res = AsyncRequestRes:New()
...
--最后天数据
function UIN10TotalLoginAwardController:_ShowLastData()
    if self._showLast then
        self._lastDataPoolGo:SetActive(true)

        if not self._lastDataItem then
            ---@type UIActivityTotalLoginAwardCell
            self._lastDataItem = self._lastDataPool:SpawnObject("UIN10TotalLoginAwardCell")
        end
...
function UIN10TotalLoginAwardController:_OnValueRemainingTime()
...
function UIN10TotalLoginAwardController:_ShowRemainingTime()
...
function UIN10TotalLoginAwardController:_GetFormatString(stamp)
...
function UIN10TotalLoginAwardController:_OnValueNextRefreshRemainingTime()
...
function UIN10TotalLoginAwardController:_ShowNextRefreshRemainingTime()
...
function UIN10TotalLoginAwardController:_GetNextRefreshFormatString(stamp)
    local formatStr = "%s <color=#%s>%s</color>"
...
function UIN10TotalLoginAwardController:_IsAllRewardUnlocked()
    for index, value in ipairs(self._data.cells) do
        ---@type DActivityTotalLoginAwardCell
