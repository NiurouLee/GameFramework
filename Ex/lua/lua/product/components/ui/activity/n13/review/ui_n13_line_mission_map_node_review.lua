---@class UIN13LineMissionMapNodeReview : UICustomWidget
_class("UIN13LineMissionMapNodeReview", UICustomWidget)
UIN13LineMissionMapNodeReview = UIN13LineMissionMapNodeReview

function UIN13LineMissionMapNodeReview:OnShow(uiParams)
    self:InitWidget()
end

function UIN13LineMissionMapNodeReview:InitWidget()
    --generated--
    ...
    self._atlas = self:GetAsset("UIN13.spriteatlas", LoadType.SpriteAtlas)
...
function UIN13LineMissionMapNodeReview:SetData(lineCfg, passInfo, cb)
    ...
    if lineCfg.WayPointType == 4 then
        --S关
        typeCfg = UIN13LineMissionControllerReview.NodeCfg[UIN13LineMissionControllerReview.SLeval]
    else
        --战斗、剧情、boss
        typeCfg = UIN13LineMissionControllerReview.NodeCfg[missionCfg.Type]
    end
    local bg = nil
...
function UIN13LineMissionMapNodeReview:_SetRed(isShow)
    local redObj = self:GetGameObject("red")
    redObj:SetActive(isShow)
end

function UIN13LineMissionMapNodeReview:BtnOnClick(go)
