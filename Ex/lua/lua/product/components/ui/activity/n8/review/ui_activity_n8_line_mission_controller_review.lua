---@class UIActivityN8LineMissionController_Review:UIController
_class("UIActivityN8LineMissionController_Review", UIController)
UIActivityN8LineMissionController_Review = UIActivityN8LineMissionController_Review

--region Helper
function UIActivityN8LineMissionController_Review:_SpawnObject(widgetName, className)
...
function UIActivityN8LineMissionController_Review:_SetRemainingTime(widgetName, descId, endTime)
...
function UIActivityN8LineMissionController_Review:Constructor()
    ---@type MissionModule
...
function UIActivityN8LineMissionController_Review:InitWidget()
    ...
    self._backBtns:SetData(
        function()
            ...
            campaignModule:CampaignSwitchState(
                true,
                UIStateType.UIActivityN8MainController_Review,
                UIStateType.UIMain,
                nil,
                self._campaign._id
            )
...
function UIActivityN8LineMissionController_Review:LoadDataOnEnter(TT, res, uiParams)
    self._campaignType = ECampaignType. CAMPAIGN_TYPE_REVIEW_N8
    self._componentId_LineMission = ECampaignReviewN8ComponentID.ECAMPAIGN_REVIEW_ReviewN8_LINE_MISSION
...
function UIActivityN8LineMissionController_Review:OnShow(uiParams)
    self._isOpen = true
    self._timerHolder = UITimerHolder:New()

    UIActivityN8LineMissionController_Review.SLeval = 999 --s关枚举id
    UIActivityN8LineMissionController_Review.Passed = 888 --通关后文本和阴影颜色
    UIActivityN8LineMissionController_Review.NodeCfg = {
        ...
        [UIActivityN8LineMissionController_Review.SLeval] = {
            [1] = {
                normal = "",
                press = "",
                lock = "",
...
function UIActivityN8LineMissionController_Review:OnHide()
    UIActivityN8LineMissionController_Review.SLeval = nil
    UIActivityN8LineMissionController_Review.NodeCfg = nil
    self._isOpen = false
    ...
    UIActivityN8LineMissionController_Review.super:Dispose()
    self._scroller:Dispose()
end

function UIActivityN8LineMissionController_Review:_Refresh()
    self:FlushNodes()
...
function UIActivityN8LineMissionController_Review:Destroy()
    self._matReq = UIWidgetHelper.DisposeLocalizedTMPMaterial(self._matReq)
end

-- function UIActivityN8LineMissionController_Review:_SetTryoutBtn()
...
function UIActivityN8LineMissionController_Review:FlushNodes()
    ...
    self._nodesPool:SpawnObjects("UIActivityN8LineMissionMapNode_Review", levelCount)
    local nodes = self._nodesPool:GetAllSpawnList()
    self._linesPool:SpawnObjects("UIActivityN8LineMissionMapLine", lineCount)
    ---@type table<number,UIActivityN8LineMissionMapLine>
    local lines = self._linesPool:GetAllSpawnList()

    local nodeIdx, lineIdx = 1, 1
    for missionID, cfg in pairs(showMission) do
        ---@type UIActivityN8LineMissionMapNode_Review
        ...
        if cfg.WayPointType ~= 4 and cfg.NeedMissionId ~= 0 then
            ...
            local n2 = cfg
            ---@type UIActivityN8LineMissionMapLine
...
function UIActivityN8LineMissionController_Review:_OnNodeClick(stageId, isStory, worldPos)
    if isStory then
        --剧情关
...
function UIActivityN8LineMissionController_Review:_EnterStage(stageId, worldPos)
...
function UIActivityN8LineMissionController_Review:_CheckSerialAutoFightShow(stageType, stageId)
...
function UIActivityN8LineMissionController_Review:ShowSerialRewards()
    self:ShowDialog("UISerialAutoFightInfo", OpenUISerialFightInfoState.Finished)
end

function UIActivityN8LineMissionController_Review:PlotEndCallback(stageId)
...
function UIActivityN8LineMissionController_Review:AttachEvents()
    self:AttachEvent(GameEventType.ActivityCloseEvent, self._CheckActivityClose)
    self:AttachEvent(GameEventType.OnUIGetItemCloseInQuest, self.OnUIGetItemCloseInQuest)
end

function UIActivityN8LineMissionController_Review:_CheckActivityClose(id)
    if self._campaign and self._campaign._id == id then
        self:SwitchState(UIStateType.UIMain)
    end
end

function UIActivityN8LineMissionController_Review:OnUIGetItemCloseInQuest(type)
...
--region help
function UIActivityN8LineMissionController_Review:_ShowUIGetItemController(rewards)
    self:ShowDialog(
        "UIGetItemController",
        rewards,
        function()
