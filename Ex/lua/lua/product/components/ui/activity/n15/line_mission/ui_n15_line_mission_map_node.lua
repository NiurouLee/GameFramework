---@class UIN15LineMissionMapNode : UICustomWidget
_class("UIN15LineMissionMapNode", UICustomWidget)
UIN15LineMissionMapNode = UIN15LineMissionMapNode

function UIN15LineMissionMapNode:OnShow(uiParams)
    self:InitWidget()
end

function UIN15LineMissionMapNode:InitWidget()
    --generated--
...
function UIN15LineMissionMapNode:SetData(lineCfg, passInfo, cb)
    ...
    if lineCfg.WayPointType == 4 then
        --S关
        typeCfg = UIN15LineMissionController.NodeCfg[UIN15LineMissionController.SLeval]
    else
        --战斗、剧情、boss
        typeCfg = UIN15LineMissionController.NodeCfg[missionCfg.Type]
    end
    local bg = nil
...
function UIN15LineMissionMapNode:_SetRed(isShow)
    local redObj = self:GetGameObject("red")
    redObj:SetActive(isShow)
end

function UIN15LineMissionMapNode:btnOnClick(go)
