---@class UIActivityN9LineMissionController:UIController
_class("UIActivityN9LineMissionController", UIController)
UIActivityN9LineMissionController = UIActivityN9LineMissionController

--region Helper
function UIActivityN9LineMissionController:_SpawnObject(widgetName, className)
...
function UIActivityN9LineMissionController:_SetRemainingTime(widgetName, descId, endTime, tickCallback, stopCallback)
...
function UIActivityN9LineMissionController:_SetRemainingTime_Action(
    widgetName,
    descId,
    endTime,
    tickCallback,
...
function UIActivityN9LineMissionController:_SetIcon(widgetName, icon)
...
function UIActivityN9LineMissionController:_SetText(widgetName, str)
...
function UIActivityN9LineMissionController:Constructor()
    ---@type MissionModule
...
function UIActivityN9LineMissionController:InitWidget()
...
function UIActivityN9LineMissionController:LoadDataOnEnter(TT, res, uiParams)
    self._campaignType = ECampaignType.CAMPAIGN_TYPE_N9
    self._componentId_LineMission = ECampaignN9ComponentID.ECAMPAIGN_N9_LEVEL_COMMON
    self._componentId_LineMissionFixteam = ECampaignN9ComponentID.ECAMPAIGN_N9_LEVEL_FIXTEAM
...
function UIActivityN9LineMissionController:OnShow(uiParams)
    self._isOpen = true
    self._timerHolder = UITimerHolder:New()

    UIActivityN9LineMissionController.SLeval = 999 --s关枚举id
    UIActivityN9LineMissionController.Passed = 888 --通关后文本和阴影颜色
...
function UIActivityN9LineMissionController:OnHide()
    ...
    UIActivityN9LineMissionController.super:Dispose()
    self._scroller:Dispose()
end

function UIActivityN9LineMissionController:_Refresh()
    self:FlushNodes()
...
function UIActivityN9LineMissionController:_SetTimeInfo()
    local endTime = self._line_component:GetComponentInfo().m_close_time
    self:_SetRemainingTime("_remainingTimePool", nil, endTime)
end

function UIActivityN9LineMissionController:_SetActionPoint()
    local componentId = ECampaignN9ComponentID.ECAMPAIGN_N9_ACTION_POINT
    ...
    if self._endTime_ActionPoint == endTime then
        Log.info("UIActivityN9LineMissionController:_SetActionPoint() Same endTime")
    elseif n1 >= n2 then
        Log.info("UIActivityN9LineMissionController:_SetActionPoint() n1 >= n2")
    else
...
function UIActivityN9LineMissionController:_ReloadAndRefreshActionPoint()
    Log.info("UIActivityN9LineMissionController:_ReloadAndRefreshActionPoint()")
    self:StartTask(
        function(TT)
            YIELD(1)
...
function UIActivityN9LineMissionController:_SetTryoutBtn()
...
function UIActivityN9LineMissionController:_SetExchangeBtn()
...
function UIActivityN9LineMissionController:FlushNodes()
    ...
    ---@type table<number,UIActivityN9LineMissionMapLine>
    local lines = self._linesPool:GetAllSpawnList()

    local nodeIdx, lineIdx = 1, 1
    for missionID, cfg in pairs(showMission) do
        ---@type UIActivityN9LineMissionMapNode
        ...
        if cfg.WayPointType ~= 4 and cfg.NeedMissionId ~= 0 then
            ...
            local n2 = cfg
            ---@type UIActivityN9LineMissionMapLine
...
function UIActivityN9LineMissionController:_onNodeClick(stageId, isStory, worldPos)
    if isStory then
        --剧情关
...
function UIActivityN9LineMissionController:_EnterStage(stageId, worldPos)
...
function UIActivityN9LineMissionController:PlotEndCallback(stageId)
...
function UIActivityN9LineMissionController:_CheckSerialAutoFightShow(stageType, stageId)
...
function UIActivityN9LineMissionController:ShowSerialRewards()
    self:ShowDialog("UISerialAutoFightInfo", OpenUISerialFightInfoState.Finished)
end

--region Event Callback
--活动说明
function UIActivityN9LineMissionController:ActionPointBtnOnClick(go)
    local componentId = ECampaignN9ComponentID.ECAMPAIGN_N9_ACTION_POINT

    ---@type ActionPointComponent
    local component = self._campaign:GetComponent(componentId)

    self:ShowDialog("UIActivityN9ActionPointDetail", component:GetItemIcon())
end
--endregion

--region AttachEvent
function UIActivityN9LineMissionController:AttachEvents()
    self:AttachEvent(GameEventType.ActivityCloseEvent, self._CheckActivityClose)
    self:AttachEvent(GameEventType.OnUIGetItemCloseInQuest, self.OnUIGetItemCloseInQuest)
end

function UIActivityN9LineMissionController:_CheckActivityClose(id)
    if self._campaign and self._campaign._id == id then
        self:SwitchState(UIStateType.UIMain)
    end
end

function UIActivityN9LineMissionController:OnUIGetItemCloseInQuest(type)
