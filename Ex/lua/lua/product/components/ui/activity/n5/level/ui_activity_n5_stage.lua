---@class UIActivityN5Stage:UIController
_class("UIActivityN5Stage", UIController)
UIActivityN5Stage = UIActivityN5Stage
...
function UIActivityN5Stage:_GetComponents()
    --UI
    --region chapter
...
function UIActivityN5Stage:OnShow(uiParams)
...
--region UIActivityN5Stage 剧情回顾
function UIActivityN5Stage:_SetStoryBtn()
...
function UIActivityN5Stage.GetAutoOpenState(matchType, stageId)
    ---@type RoleModule
...
function UIActivityN5Stage.SetAutoOpenState(matchType, stageId, isOpen)
    ---@type RoleModule
    local roleModule = GameGlobal.GetModule(RoleModule)
    local pstId = roleModule:GetPstId()
    local playerPrefsKey = pstId .. "AutoOpenState" .. matchType --XXXXXXXXAutoOpenState1_4006050
...
function UIActivityN5Stage:InitAutoBtnState()
...
function UIActivityN5Stage:autoFightBtnOnClick()
...
function UIActivityN5Stage:OnHide()
...
function UIActivityN5Stage:InitAwards(missionCfg)
...
function UIActivityN5Stage:ProcessAward(missionCfg)
    local awards = {}
...
function UIActivityN5Stage:HasPassThreeStar(missionCfg)
    if not self._missionFinishInfo then
        return false
    end
...
function UIActivityN5Stage:GetSortedArr(awardType, cfg, stageAwardType)
...
---根据路点id初始化
function UIActivityN5Stage:Init()
    ...
    if self._powerID == RoleAssetID.RoleAssetPhyPoint then
        ...
        ---@type UIActivityN5PointInfo
        local pointPool = self._pointPool:SpawnObject("UIActivityN5PointInfo")
...
function UIActivityN5Stage:Flush(missionCfg)
    ---@type UIStageChapter
    ...
    --条件
    if self._campComp:GetComponentType() == CampaignComType.E_CAMPAIGN_COM_TREE_MISSION then
        -- UIActivityN5Stage E_CAMPAIGN_COM_TREE_MISSION 时隐藏条件
...
function UIActivityN5Stage:btnFightOnClick(go)
    local enough = false
...
function UIActivityN5Stage:UpdateCondition(three_star_condition, conditions)
...
function UIActivityN5Stage:ShowTips(itemId, pos)
    self._tips:SetData(itemId, pos)
end

function UIActivityN5Stage:bgOnClick()
    self:CloseDialog()
end

function UIActivityN5Stage:threeStarTipsBtnOnClick(go)
    --GameGlobal.UAReportForceGuideEvent("UIStageClick", {"threeStarTipsBtnOnClick"}, true)
    self:ShowDialog("UIThreeStarTips")
end

function UIActivityN5Stage:btnUnknownOnClick(go)
    local buffData = {}
    buffData.name = ""
    buffData.des = ""
...
function UIActivityN5Stage:btnPlotOnClick()
    local storyList = DiscoveryStoryList:New()
    storyList:Init(self._missionID)
...
--检查可开启连续自动战斗
function UIActivityN5Stage:_CanAutoFight()
    local tipsStr = ""
    ...
    if enableParam == CampainMissionCanSerialAutoFightType.E_CAMPAIGN_MISSION_CAN_SERIAL_AUTO_FIGHT_NEED_UNLOCK then
        if self._missionFinishInfo and self._missionFinishInfo.pass_without_help == 1 then
            return true
        else
            return false, tipsStr
        end
...
--检查显示熟练的智慧按钮
function UIActivityN5Stage:_CheckShowWordBuff()
    local missionCfg = Cfg.cfg_campaign_mission[self._missionID]
    if not missionCfg then
        return false
    end
