---@class UIN28AVGGraphNodePool:UICustomWidget
    ...
    self:StartTask(
        function(TT)
            if self.node:IsHideNew() then
                ...
                local avgStoryMissionInfo = AVGStoryMissionInfo:New() --请求更新隐藏结点的New标记时，隐藏结点肯定没打过，N28AVGData:GetServerNodeDataByNodeId(nodeId)肯定返回nil，所以直接New一个AVGStoryMissionInfo即可
