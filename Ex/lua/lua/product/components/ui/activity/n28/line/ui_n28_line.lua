...
function UIN28Line:LoadDataOnEnter(TT, res, uiParams)
    ...
    self._campaignType = ECampaignType.CAMPAIGN_TYPE_N28
    self._componentId_LineMission = ECampaignN28ComponentID.ECAMPAIGN_N28_LINE_MISSION
    -- 获取活动，以及本窗口需要的组件
    self._campaign = UIActivityHelper.LoadDataOnEnter(TT,res,self._campaignType,{self._componentId_LineMission})
...
function UIN28Line:FlushNodes()
    ...
    ---@type table<number,UIN28LineMapNode>
    local nodes = self._nodesPool:GetAllSpawnList()
    ---@type table<number,UIN28LineMapLine>
    local lines = self._linesPool:GetAllSpawnList()

    local nodeIdx, lineIdx = 1, 1

    for missionID, cfg in pairs(showMission) do
        ---@type UIN28LineMapNode
