---@class UIN28LineMapNode : UICustomWidget
_class("UIN28LineMapNode", UICustomWidget)
UIN28LineMapNode = UIN28LineMapNode

function UIN28LineMapNode:OnShow(uiParams)
    self:InitWidget()
end

function UIN28LineMapNode:InitWidget()
    --generated--
...
function UIN28LineMapNode:GetNodeCfg()
    local NodeCfg =
    {
...
function UIN28LineMapNode:SetAtlas(atlas)
    self._atlas = atlas
end

---@param passInfo cam_mission_info
function UIN28LineMapNode:SetData(lineCfg, passInfo, cb)
    ...
    if lineCfg.MapPosY >= 0 then
        self._anim:Play("uieff_UIN28LineMapNode_up")
    else
        self._anim:Play("uieff_UIN28LineMapNode_down")
    end
end

function UIN28LineMapNode:_SetRed(isShow)
    local redObj = self:GetGameObject("red")
    redObj:SetActive(isShow)
end

function UIN28LineMapNode:BtnOnClick(go)
