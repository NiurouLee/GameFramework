--
---@class UIHauteCoutureDrawProbabiltyItemKR : UICustomWidget
_class("UIHauteCoutureDrawProbabiltyItemKR", UICustomWidget)
UIHauteCoutureDrawProbabiltyItemKR = UIHauteCoutureDrawProbabiltyItemKR

function UIHauteCoutureDrawProbabiltyItemKR:Constructor()
    self._drawTimes = 0
    self._rareLevel = 0
    self._atlas = self:GetAsset("UIHauteCoutureKR.spriteatlas", LoadType.SpriteAtlas)
end

function UIHauteCoutureDrawProbabiltyItemKR:OnShow(uiParams)
    self:InitWidget()
end

function UIHauteCoutureDrawProbabiltyItemKR:InitWidget()
    --generated--
...
function UIHauteCoutureDrawProbabiltyItemKR:SetData(prizeData, drawTimes, hasGot, probablity, isreplaced)
    ...
    if hasGot then
        ...
        self.detailbg.sprite = self._atlas:GetSprite("krsenior_rule_di07")
    elseif self._rareLevel - 1 > self._drawTimes then
        ...
        self.detailbg.sprite = self._atlas:GetSprite("krsenior_rule_di03")
    else
        self.detailTips:SetText(StringTable.Get("str_senior_skin_draw_rule_get_probability"))
        self.detailbg.sprite = self._atlas:GetSprite("krsenior_rule_di04")
        --概率
        if probablity then
...
function UIHauteCoutureDrawProbabiltyItemKR:FormatCount(count)
    if count < 1000 then
        return count
    end
