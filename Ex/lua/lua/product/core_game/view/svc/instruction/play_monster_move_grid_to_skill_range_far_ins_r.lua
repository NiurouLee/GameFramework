...
---@class PlayMonsterMoveGridToSkillRangeFarInstruction: BaseInstruction
_class("PlayMonsterMoveGridToSkillRangeFarInstruction", BaseInstruction)
PlayMonsterMoveGridToSkillRangeFarInstruction = PlayMonsterMoveGridToSkillRangeFarInstruction

function PlayMonsterMoveGridToSkillRangeFarInstruction:Constructor(paramList)
    --self._hitAnimName = paramList["hitAnimName"]
end

---@param casterEntity Entity
---@param phaseContext SkillPhaseContext
function PlayMonsterMoveGridToSkillRangeFarInstruction:DoInstruction(TT, casterEntity, phaseContext)
    ---@type SkillEffectResultContainer
    local skillEffectResultContainer = casterEntity:SkillRoutine():GetResultContainer()
    ---@type SkillEffectMonsterMoveGridToSkillRangeFarResult[]
    local results = skillEffectResultContainer:GetEffectResultsAsArray(SkillEffectType.MonsterMoveGridToSkillRangeFar)
...
function PlayMonsterMoveGridToSkillRangeFarInstruction:_ShowLinkLine(TT,walkResultList,element)
...
function PlayMonsterMoveGridToSkillRangeFarInstruction:_DestroyLinkLine(moveInPos)
...
function PlayMonsterMoveGridToSkillRangeFarInstruction:_DoWalk(TT, monsterEntity, walkResultList, casterIsDead)
    ---@type BoardServiceRender
    local boardServiceRender = self._world:GetService("BoardRender")
...
function PlayMonsterMoveGridToSkillRangeFarInstruction:_PlayArrivePos(TT, monsterEntity, walkRes)
    ---触发机关的表现
    ...
    local dieTrapID = walkRes:GetFlushTrapID()
    if dieTrapID then
        local trapEntity = self._world:GetEntityByID(dieTrapID)
        if trapEntity then
            ---@type TrapServiceRender
            local trapSvc = self._world:GetService("TrapRender")
            trapSvc:PlayTrapDieSkill(TT, { trapEntity })
        end
    end
...
---@param casterEntity Entity
function PlayMonsterMoveGridToSkillRangeFarInstruction:_GetMoveSpeed(casterEntity)
    ---@type ConfigService
...
function PlayMonsterMoveGridToSkillRangeFarInstruction:StartMoveAnimation(targetEntity, isMove)
