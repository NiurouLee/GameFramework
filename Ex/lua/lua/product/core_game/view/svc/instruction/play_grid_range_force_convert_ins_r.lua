require("base_ins_r")
---@class PlayGridRangeForceConvertInstruction: BaseInstruction
_class("PlayGridRangeForceConvertInstruction", BaseInstruction)
PlayGridRangeForceConvertInstruction = PlayGridRangeForceConvertInstruction

function PlayGridRangeForceConvertInstruction:Constructor(paramList)
...
function PlayGridRangeForceConvertInstruction:DoInstruction(TT, casterEntity, phaseContext)
            ...
            if posList then
                for __, pos in pairs(posList) do
                    ...
                    if gridEntity then
                        ...
                        --从最终逻辑取颜色
                        local forcePieceType = utilDataSvc:GetPieceType(pos)

                        --转色
                        self:_Convert(world, pos, forcePieceType, flushTraps, casterEntity, TT)
...
function PlayGridRangeForceConvertInstruction:_GetConvertSkillResult(world, skillEffectResultContainer, pos, dataSource)
    local nNewGridType = nil
    local flushTraps = {} --需要洗掉的机关
    local notifyBuff = true --转色是否通知buff
...
function PlayGridRangeForceConvertInstruction:_Convert(world, gridPos, newGridType, flushTraps, casterEntity, TT)
    --洗机关，直接删除
    ---@type TrapServiceRender
    local trapServiceRender = world:GetService("TrapRender")
...
function PlayGridRangeForceConvertInstruction:_ShowTrapAtPos_ConvertOccupiedGridElement(TT, world, skillResultArray, pos)
    if skillResultArray and #skillResultArray > 0 then
        ---@type TrapServiceRender
        local trapServiceRender = world:GetService("TrapRender")
