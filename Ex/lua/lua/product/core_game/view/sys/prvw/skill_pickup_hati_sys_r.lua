--[[----------------------------------------------------------
    SkillPickUpHatiSystem_Render 指令化的PickUPSystem 
]] ------------------------------------------------------------
---@class SkillPickUpHatiSystem_Render:ReactiveSystem
_class("SkillPickUpHatiSystem_Render", ReactiveSystem)
SkillPickUpHatiSystem_Render = SkillPickUpHatiSystem_Render

---@param world MainWorld
function SkillPickUpHatiSystem_Render:Constructor(world)
    self._world = world
    self._isGuide = false
    self._pickUpType = nil

    self._IsRepeatPickupFunc = {}
    self._IsRepeatPickupFunc[SkillPickUpType.Hati] = self.IsRepeatPickUP_PickGrid

    self._ProgressInvalidFunc = {}
    self._ProgressInvalidFunc[SkillPickUpType.Hati] = self.ProgressInvalidGridList_PickGrid

    self._RemovePickUpGridPos = {}
    self._RemovePickUpGridPos[SkillPickUpType.Hati] = self.RemovePickUpGridPos_PickGrid
end

---@param world World
function SkillPickUpHatiSystem_Render:GetTrigger(world)
    local c =
        Collector:New(
        {
...
---@param entity Entity
function SkillPickUpHatiSystem_Render:Filter(entity)
    ...
    if skillHandleType == SkillPickUpType.Hati then
        return true
    end
    return false
end

function SkillPickUpHatiSystem_Render:ExecuteEntities(entities)
    for i = 1, #entities do
        self:DoPickUp(entities[i])
    end
end

function SkillPickUpHatiSystem_Render:DoPickUp(entity)
    ---@type PickUpTargetComponent
    local pickUpTargetCmpt = entity:PickUpTarget()
    ...
    if table.icontains(validGridList, pickUpGridPos) then
        if not self:_HatiCheckOk(pickUpGridPos,petEntity) then
            ToastManager.ShowToast(StringTable.Get("str_battle_hati_target_illegal"))
            return
        end
        ---点选无效范围
        ---@type GuideServiceRender
...
function SkillPickUpHatiSystem_Render:_DoPickUpInstruction(TT, type, skillConfigData, casterEntity, pickUpGirdPos)
    ---@type number[]
    local taskIDList = {}
    ----@type PreviewActiveSkillService
...
function SkillPickUpHatiSystem_Render:_GetInstructSet(type, skillPreviewConfigData)
...
function SkillPickUpHatiSystem_Render:_GetPreviewContext(
    type,
    skillPreviewConfigData,
    casterEntity,
    id,
...
function SkillPickUpHatiSystem_Render:ProcessInvalidGridList(validGridList, invalidGridList)
    local fun = self._ProgressInvalidFunc[self._pickUpType]
    return fun(self, validGridList, invalidGridList)
end

function SkillPickUpHatiSystem_Render:ProgressInvalidGridList_PickGrid(validGridList, invalidGridList)
...
---@param pickUpGridPos Vector2
function SkillPickUpHatiSystem_Render:IsRepeatPickUP(allPickUpPos, pickUpGridPos)
    return self._IsRepeatPickupFunc[self._pickUpType](self, allPickUpPos, pickUpGridPos)
end

----@param allPickUpPos Vector2[]
---@param pickUpGridPos Vector2
function SkillPickUpHatiSystem_Render:IsRepeatPickUP_PickGrid(allPickUpPos, pickUpGridPos)
    return table.icontains(allPickUpPos, pickUpGridPos)
end

function SkillPickUpHatiSystem_Render:RemovePickUpGridPos_PickGrid(previewPickUpComponent, pickGridPos)
    previewPickUpComponent:RemoveGridPos(pickGridPos)
end

function SkillPickUpHatiSystem_Render:RemoveRepeatPickUpGrid(previewPickUpComponent, pickGridPos)
    self._RemovePickUpGridPos[self._pickUpType](self, previewPickUpComponent, pickGridPos)
end


function SkillPickUpHatiSystem_Render:_HatiCheckOk(pickPos,petEntity)
    ----@type UtilDataServiceShare
    local utilDataSvc = self._world:GetService("UtilData")
...
---@return Vector2[]
function SkillPickUpHatiSystem_Render:GetPosListAroundBodyArea(entity, ringCount)
