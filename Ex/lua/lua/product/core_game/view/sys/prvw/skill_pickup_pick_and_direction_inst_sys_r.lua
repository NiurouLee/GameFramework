...
---@class SkillPickUpPickAndDirectionInstructionSystem_Render:ReactiveSystem
...
---准备选择方向状态的显示，有效点击1个点，
function SkillPickUpPickAndDirectionInstructionSystem_Render:_OnReadyToSelectDirectionShow(
    petEntity,
    skillConfigData,
    pickUpGridPos,
    lastPickUpPos)
...
---已经选择方向状态的显示，有效点击2个点，
function SkillPickUpPickAndDirectionInstructionSystem_Render:_OnHadToSelectDirectionShow(
    petEntity,
    skillConfigData,
    pickUpGridPos,
    lastPickUpPos)
...
function SkillPickUpPickAndDirectionInstructionSystem_Render:IsDirValid(lastPickUpPos,curPickUpPos)
    if (curPickUpPos.x== lastPickUpPos.x+1 or curPickUpPos.x== lastPickUpPos.x-1 or curPickUpPos.x== lastPickUpPos.x)
        and curPickUpPos.y ==  lastPickUpPos.y then
        return true
    end
    if (curPickUpPos.y== lastPickUpPos.y+1 or curPickUpPos.y== lastPickUpPos.y-1 or curPickUpPos.y== lastPickUpPos.y)
            and curPickUpPos.x ==  lastPickUpPos.x then
        return true
    end
    return false
end
            ...
            if not table.Vector2Include(validGridList, pickUpGridPos) then
                ...
                if AutoPickCheckHelperRender.IsAutoFightRunning() then--自动战斗下点选异常上报
                    AutoPickCheckHelperRender.ReportAutoFightPickError(
                        ActivePickSkillCheckErrorStep.PickAndDirectionInsInvalid,ActivePickSkillCheckErrorType.None,
                        activeSkillID,pickUpGridPos)
                end
                return
            end
        else
            if not self:IsDirValid(lastPickUpPos, pickUpGridPos) then
                previewActiveSkill:PickUpInvalidGridCancelPreview(activeSkillID, petPstID)
                if AutoPickCheckHelperRender.IsAutoFightRunning() then--自动战斗下点选异常上报
                    AutoPickCheckHelperRender.ReportAutoFightPickError(
                        ActivePickSkillCheckErrorStep.PickAndDirectionInsDirInvalid,ActivePickSkillCheckErrorType.None,
                        activeSkillID,pickUpGridPos)
                end
                return
            end
...
function SkillPickUpPickAndDirectionInstructionSystem_Render:_DoPickUpInstruction(
    TT,
    type,
    skillConfigData,
    casterEntity,
...
function SkillPickUpPickAndDirectionInstructionSystem_Render:_GetPreviewContext(
    type,
    skillPreviewConfigData,
    casterEntity,
    id,
