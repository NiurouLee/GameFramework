...
public class UIFacadeInspector : OdinEditor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (m_FoldBasicInfo)
        {
            ...
            if (m_EnableSubModule)
                m_EditorData.SubModuleName = SirenixEditorFields.TextField("子模块名称", m_EditorData.SubModuleName);
            ...
            EditorGUILayout.LabelField("脚本名称: 模块名 + 子模块名 + UI名称 + Window/View",
        ...
        SirenixEditorGUI.BeginBox();
        {
            ...
            SirenixEditorGUI.BeginHorizontalToolbar();
            {
                ...
                component.IsExpanded =
                    EditorGUILayout.Foldout(component.IsExpanded, $"[{index}] {component.Name}", true);
            ...
            if (component.IsExpanded)
            {
                ...
                EditorGUILayout.BeginHorizontal();
                {
                    ...
                    GameObject selectedGO = (GameObject)EditorGUILayout.ObjectField(currentGO, typeof(GameObject), true,
                    ...
                    if (EditorGUI.EndChangeCheck() && selectedGO != currentGO)
                    {
                        ...
                        else
                        {
                            ...
                            Component[] validComponents =
                                allComponents.Where(c => c != null && !(c is Transform)).ToArray();
                    ...
                    if (targetGO != null)
                    {
                        ...
                        {
                            EditorGUILayout.LabelField("无组件", SirenixGUIStyles.RightAlignedGreyMiniLabel,
                                GUILayout.Width(fieldWidth));
                        }
                    }
                    else
                    {
                        EditorGUILayout.LabelField("请选择对象", SirenixGUIStyles.RightAlignedGreyMiniLabel,
                            GUILayout.Width(fieldWidth));
                    }
                }
                EditorGUILayout.EndHorizontal();
    ...
    private void DrawInputTypesMultiSelect(UIComponent component)
    {
        ...
        EditorGUILayout.BeginHorizontal();
        {
            ...
            if (component.SelectedInputTypes.Count > 0)
            {
                ...
                GUILayout.Label($"({selectedText})", SirenixGUIStyles.RightAlignedGreyMiniLabel,
                ...
                if (!newComponents.Any(c => c != null && c.Name == componentName))
                {
                    newComponents.Add(new UIComponent
                    {
                        ...
                        ActiveDefault = go.activeInHierarchy
                            ? ElementActiveDefault.Active
                            : ElementActiveDefault.DeActive
                    });
                }
            }
        }
        ...
        if (m_EditorData.Components != null)
        {
            for (int i = 0; i < m_EditorData.Components.Length; i++)
            {
                ...
                if (comp != null && comp.Component != null && !string.IsNullOrEmpty(comp.Name))
                {
                    ...
                    sb.AppendLine(
                        $"        public {typeName} {comp.Name} => Facade.m_RuntimeInputComponents[{i}] as {typeName};");
                }
            }
        }
    ...
    private void DrawViewConfig()
    {
        ...
        // UI层级设置
        ushort currentLayer = m_ViewConfig.Layer;
