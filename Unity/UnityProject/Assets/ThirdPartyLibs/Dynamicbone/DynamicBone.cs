...
public class DynamicBone : MonoBehaviour
{
    ...
    [Range(0, 1)]
    public float m_Stiffness = 0.03f;
    ...
    void LateUpdate()
    {
        ...
        if (m_Weight > 0 && !(m_DistantDisable && m_DistantDisabled))
            UpdateDynamicBones(m_SimulateTimeScale > 0f
                ? Time.unscaledDeltaTime * m_SimulateTimeScale : Time.deltaTime);
    }
    ...
    void UpdateDynamicBones(float t)
    {
        ...
        if (m_Root == null)
        {
            Debug.LogError("DynamicBone.UpdateDynamicBones : m_Root == null : " + gameObject.name);
            return;
        }
#endif
        ...
        m_ObjectScale = Mathf.Abs(tran.lossyScale.x);
        m_ObjectMove = pos - m_ObjectPrevPosition;
        m_ObjectPrevPosition = pos;
    ...
    void AppendParticles(Transform b, int parentIndex, float boneLength)
    {
        ...
        if (b != null)
        {
            ...
            {
                bool exclude = IsExclude(b.GetChild(i));
    ...
    bool IsExclude(Transform transform)
    {
        if (m_Exclusions != null)
        {
            for (int j = 0; j < m_Exclusions.Count; ++j)
    ...
    {
        if (m_Exclusions == null)
            m_Exclusions = new List<Transform>();
        m_Exclusions.Add(transform);
    }
    ...
    public void SimulateTimeScale(float simulateTimeScale)
    {
        m_SimulateTimeScale = simulateTimeScale;
    }
}
